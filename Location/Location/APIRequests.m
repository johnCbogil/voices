...
//    self.sfConnection = [[NSURLConnection alloc] initWithRequest:getRequest delegate:self];
    ...
    NSURLSession *session = [NSURLSession sharedSession];
    NSString *urlString = [NSString stringWithFormat:@"https://congress.api.sunlightfoundation.com/legislators/locate?latitude=%.8f&longitude=%.8f&apikey=%@", latitude, longitude, SFKEY];
    NSURL *url = [NSURL URLWithString: [urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
...
//    self.photoConnection = [[NSURLConnection alloc] initWithRequest:getRequest delegate:self];
    ...
    NSURLSession *session = [NSURLSession sharedSession];
    NSString *urlString = [NSString stringWithFormat:@"http://theunitedstates.io/images/congress/450x550/%@.jpg", bioGuide];
    NSURL *url = [NSURL URLWithString: [urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    ...
    [[session dataTaskWithURL:url
            completionHandler:^(NSData *data,
                ...
                if ([data length] < 15000) {
                    ...
                    [self.congressmenPhotos addObject:[UIImage imageWithData:data]];
...
//        self.googleCivConnection = [[NSURLConnection alloc] initWithRequest:googleGetRequest delegate:self];
        ...
        NSURLSession *session = [NSURLSession sharedSession];
//        NSString *urlString = [NSString stringWithFormat:@"http://theunitedstates.io/images/congress/450x550/%@.jpg", bioGuide];
        NSURL *url = [NSURL URLWithString: [encodedString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
...
//    }
//     if (connection == self.googleCivConnection){
//        
